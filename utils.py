...
def position(host, rack_gr):  # определяет позицию в стойке
    return int(host[-2:]) + first_unit(host, rack_gr)


def first_unit(host, rack_gr):  # определяет позицию в стойке
    rack_gr = str(rack_gr).lower()
    if rack_gr[0:5] == 'icva2':
        if host[-6:-2] == '6129':  # стойка с квм переключателем
            ...
            return 8
    elif rack_gr[0:5] == 'icva3':
        return 9
    elif rack_gr[0:5] == 'icva1':
        # стойки хостинга
        if host[-6:-2] == '5360' or host[-6:-2] == '5280' or host[-6:-2] == '5040' or host[-6:-2] == '5160':
            return 13
        elif rack_gr[-3:] == '_00' or rack_gr[-3:] == '_11':
            return 9
        else:
            return 7
    elif rack_gr[0:5] == 'icva_':
...
def rack_group(row):
    ...
    if row[0] == '5':
        first = 'ICVA1_'
        second = f"0{number_}"[-2:]
    elif row[0] == '6' and number_ <= 4:
        first = 'ICVA2_'
        second = str(number_)
    elif row[0] == '6' and number_ > 4:
        first = 'ICVA3_'
        number_ = int(number_ - 4)
        second = str(number_)
    return first + second
